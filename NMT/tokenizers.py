...
class SPMTokenizer():
    def __init__(
        ...
        self.spm = spm.SentencePieceProcessor(model_file=self.spm_name + ".model")
        with open(self.spm_model + ".vocab") as inf:
    ...
    def batch_detokenize(self, batch, remove_special_toks=False):
        batch = [
            self.spm_detokenize(seq, remove_special_toks=remove_special_toks)
            for seq in batch
        ]
        return batch
