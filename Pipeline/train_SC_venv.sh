#!/bin/bash
#
# train_SC_venv.sh - Venv version of train_SC.sh
#
# This is a fork of train_SC.sh that uses Python virtual environments (venv)
...
#
# Arguments:
#   $1: Configuration file (same as train_SC.sh)
#   $2: Path to sound venv (e.g., ./venv_sound)
#   $3: Path to copper venv (e.g., ./venv_copper)
#
...
if [ $# -ne 3 ]; then
    ...
    echo ""
    echo "Usage: $0 <config_file> <venv_sound_path> <venv_copper_path>"
    echo ""
    echo "Example:"
    echo "  $0 Pipeline/cfg/SC/my_config.cfg ./venv_sound ./venv_copper"
    echo ""
    echo "The original train_SC.sh uses conda and only requires the config file."
    echo "This venv version requires you to also specify the venv paths."
    exit 1
fi
...
# Verify venv directories exist
if [ ! -d "$VENV_SOUND" ]; then
    echo "Error: Sound venv not found at: $VENV_SOUND"
    echo "Create it with: python3.10 -m venv $VENV_SOUND && source $VENV_SOUND/bin/activate && pip install -r sound.requirements.txt"
    exit 1
fi

if [ ! -d "$VENV_COPPER" ]; then
    echo "Error: Copper venv not found at: $VENV_COPPER"
    echo "Create it with: python3.8 -m venv $VENV_COPPER && source $VENV_COPPER/bin/activate && pip install -r copper.requirements.txt"
    exit 1
fi

# Convert to absolute paths
VENV_SOUND=$(cd "$VENV_SOUND" && pwd)
VENV_COPPER=$(cd "$VENV_COPPER" && pwd)

echo "Starting train_SC_venv.sh-----------"
echo "Using venv (not conda)"
echo "  Sound venv:  $VENV_SOUND"
echo "  Copper venv: $VENV_COPPER"
date
echo "-------------------------------"
...
echo "#-- #                 1) ARGUMENTS                # --#"
echo "    ###############################################"
source $CONFIG_FILE # .cfg file from from Pipeline/cfg/SC
